...
const PostList = ({ posts, selectPost }) => {
	...
	return (
        <Grid container direction="column"  spacing={1} className={classes.postListRoot}>
            {posts.map((_post, _index) => {
                return (
                    <Grid item>
                        <SelectPost key={_index} selectPost={selectPost} post={_post} />
                    </Grid>
                );
            })}
        </Grid>
    ...
    return (
		<div onClick={() => selectPost(post)}>
			<Grid container direction="column" className={classes.postRoot}>
				<Grid item>
					<Typography color="secondary">{post.index + ':'}</Typography>
				</Grid>
				<Grid item>
					<Typography
						style={{ width: '280px' }}
